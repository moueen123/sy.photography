...
// collections photos (for the works-showcase overlay)
...
// --- collections videos ------------------------------------------------
// Rename these to match your real files. A poster image is optional but
// recommended: without one the browser has to download the first frame
// itself, which is slower and sometimes shows a black tile on iOS.
...
// A video album counts its videos; a photo album keeps its own count.
...
const mediaLabelOf = (album) => {
  ...
  if (album.mediaType === "video") return count === 1 ? "Video" : "Videos";
...
// Card width, gap, and how much of the neighboring ("peeking") cards shows
// all scale with viewport instead of being fixed pixel values. peekVisible
// is the number of px of a neighbor card that's allowed to show at the
// screen edge — on mobile this stays small so neighbors don't overlap the
// centered card; on desktop it grows back to the original ~half-card peek.
...
const getResponsiveCardDims = () => {
  ...
  if (vw < 480) {
    // small phones — card fills most of the viewport, neighbors only show
    // a small sliver at the very edge so they don't collide with center
    const width = Math.round(vw * 0.8);
    return { width, gap: 12, peekVisible: 14 };
  }
  if (vw < 768) {
    // large phones / small tablets
    const width = Math.min(460, Math.round(vw * 0.74));
    return { width, gap: 18, peekVisible: 26 };
  }
  if (vw < 1024) {
    // tablets
    const width = Math.min(560, Math.round(vw * 0.6));
    return { width, gap: 28, peekVisible: 80 };
  }
  if (vw < 1280) {
    // small laptops
    const width = Math.min(620, Math.round(vw * 0.5));
    return { width, gap: 34, peekVisible: 160 };
  }
  // large desktops — original design size & peek amount
  return {
    width: MAX_CARD_WIDTH,
    gap: MAX_TRAIN_GAP,
    peekVisible: MAX_CARD_WIDTH / 2,
...
// Given the current viewport width, card width, and how much of a neighbor
// should peek into view, returns the x-offset (from center) at which the
// immediate neighbor card should sit.
const getPeekOffset = (viewportWidth, cardWidth, peekVisible) => {
  return viewportWidth / 2 + cardWidth / 2 - peekVisible;
};

// --- Lazy-loaded photo tile --------------------------------------------
// Only the first few photos load right away; the rest load as you scroll
// toward them. The min-height placeholder keeps every tile from collapsing
// to 0px before its image arrives (otherwise the browser would think all
// 52 tiles are "in view" at once and load them all immediately).
const EAGER_PHOTO_COUNT = 6;

const PhotoTile = ({ src, alt, eager }) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        width: "100%",
        minHeight: loaded ? undefined : "280px",
      }}
    >
      <img
        src={src}
...
const HeroSequence = () => {
  ...
  const titleRef = useRef(null); // "Collections." heading — only visible once the cards phase starts
  const wsSectionRef = useRef(null); // the works-showcase overlay panel
  const cardRefs = useRef([]);

  // NEW: full-screen video player refs
  const playerOverlayRef = useRef(null);
  const playerFrameRef = useRef(null);
  const playerVideoRef = useRef(null);
  const previewVideoRefs = useRef([]);
  // Entering fullscreen fires a resize; if the debounced resize handler runs
  // ScrollTrigger.refresh(), the pin-spacer is rebuilt around this container
  // and the browser drops out of fullscreen. These guard against that.
  ...
  // responsive card width/gap live in refs (for GSAP math) and in
  // state (so the JSX can size the card + highlight elements to match).
  ...
  // works-showcase overlay state
  const [selectedAlbumIndex, setSelectedAlbumIndex] = useState(null); // which album's content to render
  const [isWsOpen, setIsWsOpen] = useState(false); // drives the slide animation direction

  // NEW: full-screen video player state
  const [activeVideo, setActiveVideo] = useState(null); // { src, poster, title }
  const [durations, setDurations] = useState({}); // src -> seconds, filled in from metadata
  ...
  const TOTAL_SCROLL_DISTANCE = FRAME_SCROLL_DISTANCE + CARDS_SCROLL_DISTANCE;
  const FRAME_FRACTION = FRAME_SCROLL_DISTANCE / TOTAL_SCROLL_DISTANCE;
  const NUM_CARDS = albums.length;
  const SEGMENT_SIZE = 1 / NUM_CARDS;

  // STEP depends on the current responsive card width/gap, so it's now a
  // function that reads the live refs instead of a fixed constant.
  const getStep = () => cardWidthRef.current + cardGapRef.current;

  const [contactHovered, setContactHovered] = useState(false); // drives Contact button's corner-bracket hover effect

  const { loadingProgress, isLoaded, getFrame, getFrameIndexFromProgress } =
    useFrameSequence({
      frameCount: FRAME_COUNT,
      framePathTemplate: "/hero/frames/frame.{index}.jpeg",
      startFrame: 1,
      paddingLength: 3,
    });

  const updateFrame = (progress) => {
    const frameIndex = getFrameIndexFromProgress(progress);
    const frame = getFrame(frameIndex);

    if (frame && frame !== currentFrameRef.current) {
      currentFrameRef.current = frame;
      setCurrentFrame(frame);
    }
  };

  const targetXForCard = (i, centerIndex) => {
    if (centerIndex === -2) return farOffsetRef.current;
    if (i === centerIndex) return 0;

    const step = getStep();

    if (i > centerIndex) {
      const distanceAhead = i - centerIndex;
      return peekOffsetRef.current + (distanceAhead - 1) * step;
    }

    const distanceBehind = centerIndex - i;
    return -(peekOffsetRef.current + (distanceBehind - 1) * step);
  };

  const applyCenterIndex = (newCenterIndex) => {
    if (centerIndexRef.current === newCenterIndex) return;
    centerIndexRef.current = newCenterIndex;

    cardRefs.current.forEach((el, i) => {
      if (!el) return;
      gsap.to(el, {
        x: targetXForCard(i, newCenterIndex),
    ...
    setDisplayedIndex(newCenterIndex >= 0 ? newCenterIndex : -1);
  };

  // open the WS section for a specific album
  const openWsSection = (index) => {
    setSelectedAlbumIndex(index);
    setIsWsOpen(true);
  };

  // close it — content (selectedAlbumIndex) is cleared only AFTER the
  // slide-out finishes
  ...
  // Rewinding alone leaves the first decoded frame on screen, so the poster
  // never comes back. load() drops the decoded frame and re-shows the poster.
  ...
  const openVideo = (video) => {
    // stop any muted grid previews so two soundtracks never overlap
    previewVideoRefs.current.forEach(resetPreview);
  ...
  const closeVideo = () => {
    ...
    const finish = () => {
      setActiveVideo(null);
      // any resize that happened while the player was open was ignored, so
      // catch up now that the hero is visible again
      requestAnimationFrame(() => ScrollTrigger.refresh());
    };

    if (playerVideoRef.current) playerVideoRef.current.pause();
    ...
    if (prefersReducedMotion || !playerOverlayRef.current) {
      finish();
      return;
    }

    gsap.to(playerFrameRef.current, {
      scale: 0.96,
      y: 16,
      opacity: 0,
      duration: 0.25,
    ...
    gsap.to(playerOverlayRef.current, {
      opacity: 0,
      duration: 0.3,
      ease: "power2.in",
      overwrite: "auto",
  ...
  // entrance animation for the player
  useGSAP(() => {
    if (!activeVideo || !playerOverlayRef.current) return;
    ...
    gsap.fromTo(
      ...
      },
      {
        ...
        y: 0,
        opacity: 1,
        duration: prefersReducedMotion ? 0 : PLAYER_DURATION,
        ease: PLAYER_EASE,
        overwrite: "auto",
      },
    );
  ...
  // keep the resize guard in sync with the open player
  useEffect(() => {
    activeVideoRef.current = activeVideo;
  }, [activeVideo]);

  // track fullscreen so the resize handler can stand down while it's active
  useEffect(() => {
  ...
  // Esc closes the player first, then the collection
  useEffect(() => {
    const onKeyDown = (e) => {
  ...
  }, [activeVideo, isWsOpen]);

  // drives the WS section's slide in/out, purely off isWsOpen
  useGSAP(() => {
    if (!wsSectionRef.current) return;

    if (isWsOpen) {
      gsap.to(wsSectionRef.current, {
        y: "0%",
        duration: prefersReducedMotion ? 0 : WS_DURATION,
        ease: WS_EASE,
        overwrite: "auto",
      });
    } else {
      gsap.to(wsSectionRef.current, {
        ...
        overwrite: "auto",
        onComplete: () => {
          // Only clear the content once it's fully off-screen again
          setSelectedAlbumIndex(null);
        },
      });
    }
  }, [isWsOpen, prefersReducedMotion]);
  ...
  useGSAP(() => {
    ...
    const { width, gap, peekVisible } = getResponsiveCardDims();
    cardWidthRef.current = width;
    cardGapRef.current = gap;
    setCardWidth(width);

    peekOffsetRef.current = getPeekOffset(viewportWidth, width, peekVisible);
    farOffsetRef.current = viewportWidth / 2 + width / 2 + 40;

    centerIndexRef.current = -2;
    cardRefs.current.forEach((el) => {
      if (el) gsap.set(el, { x: farOffsetRef.current });
    });

    gsap.set(canvasWrapperRef.current, { opacity: 1 });
    gsap.set(highlightCardRef.current, { opacity: 0 });
    gsap.set(titleRef.current, { opacity: 0 }); // hidden during the frame phase
    gsap.set(wsSectionRef.current, { y: "100%" }); // starts fully below viewport

    scrollTriggerRef.current = ScrollTrigger.create({
      trigger: container,
      start: "top top",
      end: `+=${window.innerHeight * TOTAL_SCROLL_DISTANCE}`,
      pin: true,
      pinSpacing: true,
      anticipatePin: 1,
      scrub: 0.5,
      onUpdate: (self) => {
        ...
        if (progress <= FRAME_FRACTION) {
          const frameProgress = progress / FRAME_FRACTION;
          updateFrame(frameProgress);
          ...
          const cardsProgress =
            (progress - FRAME_FRACTION) / (1 - FRAME_FRACTION);

          const canvasOpacity = gsap.utils.clamp(0, 1, 1 - cardsProgress / 0.2);
          gsap.set(canvasWrapperRef.current, { opacity: canvasOpacity });

          const highlightOpacity = gsap.utils.clamp(
            0,
            1,
            cardsProgress / HIGHLIGHT_FADE_FRACTION,
          );
          gsap.set(highlightCardRef.current, { opacity: highlightOpacity });
          gsap.set(titleRef.current, { opacity: highlightOpacity });

          const rawIndex = cardsProgress / SEGMENT_SIZE;
          const activeIndex = gsap.utils.clamp(
            0,
            NUM_CARDS - 1,
            Math.floor(rawIndex),
          );
          const localProgress = gsap.utils.clamp(0, 1, rawIndex - activeIndex);
          const crossedHalf = localProgress >= 0.5;

          const centerIndex = crossedHalf ? activeIndex : activeIndex - 1;
          applyCenterIndex(centerIndex);
  ...
  // keep card width/gap/peek and offsets in sync with the viewport.
  // Debounced so it doesn't thrash on every pixel of a drag-resize, and
  // repositions cards instantly (gsap.set, no animation) so the train
  // never looks broken mid-resize.
  useEffect(() => {
    if (!isLoaded) return;

    const handleResize = () => {
      // Fullscreen (and the open player generally) triggers resize events.
      // Re-laying out the pinned container here would kick the video out of
      // fullscreen, and the card train isn't visible anyway.
      if (isFullscreenRef.current || activeVideoRef.current) return;
      ...
      resizeTimeoutRef.current = setTimeout(() => {
        ...
        peekOffsetRef.current = getPeekOffset(
          viewportWidth,
          width,
          peekVisible,
        );
        ...
        cardRefs.current.forEach((el, i) => {
          if (!el) return;
          gsap.set(el, {
            x: targetXForCard(i, centerIndexRef.current),
          });
        });
  ...
  useEffect(() => {
    if (prefersReducedMotion && isLoaded) {
      const lastFrame = getFrame(FRAME_COUNT);
      setCurrentFrame(lastFrame);
      setHeroComplete(true);
      setDisplayedIndex(NUM_CARDS - 1);

      if (canvasWrapperRef.current)
        gsap.set(canvasWrapperRef.current, { opacity: 0 });
      if (highlightCardRef.current)
        gsap.set(highlightCardRef.current, { opacity: 1 });
      if (titleRef.current) gsap.set(titleRef.current, { opacity: 1 });
      if (wsSectionRef.current) gsap.set(wsSectionRef.current, { y: "100%" });

      const viewportWidth = window.innerWidth;
      const { width } = getResponsiveCardDims();
      cardWidthRef.current = width;
      setCardWidth(width);
      const far = viewportWidth / 2 + width / 2 + 40;
      cardRefs.current.forEach((el, i) => {
        if (!el) return;
        gsap.set(el, { x: i === NUM_CARDS - 1 ? 0 : -far });
      });
    }
  }, [prefersReducedMotion, isLoaded]);

  useEffect(() => {
    document.body.classList.toggle("ws-open", isWsOpen);
  ...
  const displayedAlbum = displayedIndex >= 0 ? albums[displayedIndex] : null;
  const selectedAlbum =
    selectedAlbumIndex !== null ? albums[selectedAlbumIndex] : null;
  ...
  return (
    ...
    >
      {!isLoaded && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <p className="metadata text-warm-white/60 mb-2">
              {loadingProgress < 100 ? `${loadingProgress} / 100` : "LOADING"}
            </p>
            <div className="w-32 h-px bg-warm-white/20">
              <div
                className="h-full bg-warm-white/60 transition-all duration-300"
      ...
      {/* Contact Now button — scales down and moves closer to the edge on small screens */}
      <button
        onClick={() => {
          window.open(
      ...
      >
        <div className="relative flex items-center justify-center gap-2 sm:gap-4 bg-black/40 backdrop-blur-sm border border-warm-white/20 rounded-full transition-all duration-300 group-hover:border-warm-white/60 group-hover:bg-black/70 w-[170px] h-[56px] px-5 sm:w-[260px] sm:h-[80px] sm:px-8">
          ...
          {/* Corner brackets — same viewfinder style as the highlight card / nav links, fade in on hover */}
          <span
            ...
            style={{ opacity: contactHovered ? 1 : 0 }}
          />
          <span
      ...
      <div
        ...
        style={{
          paddingTop: "calc(var(--navbar-h, 90px) + clamp(16px, 4vw, 40px))",
          pointerEvents: "none",
        }}
      >
          ...
          >
            <img
              ...
              className={`w-full h-full ${
                [3, 4, 5].includes(album.id) ? "object-contain" : "object-cover"
              }`}
            />
      ...
      {displayedAlbum && (
        <div className="absolute bottom-8 left-6 sm:bottom-16 sm:left-16 z-30 max-w-[80vw]">
          ...
          <p className="text-warm-white/60 text-base sm:text-lg md:text-xl mt-2">
            {String(mediaCountOf(displayedAlbum)).padStart(2, "0")}{" "}
            {mediaLabelOf(displayedAlbum)}
          </p>
        </div>
      )}
      ...
      {/* Works Showcase (WS) section — full-screen overlay, slides up from
          below the viewport on open, reverses on close. */}
      <div
        ref={wsSectionRef}
        className="absolute inset-0 z-50 bg-black overflow-y-auto overscroll-contain"
        style={{ pointerEvents: isWsOpen ? "auto" : "none" }}
      >
        {selectedAlbum && (
          <>
            {/* Close button — sticky so it stays on screen while the
                collection scrolls (position: fixed doesn't work here because
                the overlay has a GSAP transform). The h-0 wrapper takes no
                space, so the layout below is unchanged. */}
            <div className="sticky top-0 z-50 h-0">
              <button
                onClick={closeWsSection}
                aria-label="Close"
            ...
            <div className="min-h-screen px-4 sm:px-8 md:px-16 pt-16 sm:pt-20 md:pt-24 pb-16 sm:pb-20 md:pb-24">
              ...
              <div className="flex flex-col items-center">
                ...
                <h2 className="text-warm-white text-3xl sm:text-4xl md:text-6xl font-light text-center px-8">
                  <div className="spacer w-full h-6 sm:h-8 md:h-10 "></div>
                  {selectedAlbum.title}
                </h2>
                <div className="spacer w-full h-6 sm:h-8 md:h-10 "></div>
                {/* Media count */}
                <p className="text-warm-white/60 text-lg sm:text-xl md:text-2xl mt-4 sm:mt-6">
                  {String(mediaCountOf(selectedAlbum)).padStart(2, "0")}{" "}
                  {mediaLabelOf(selectedAlbum)}
                </p>
              </div>

              {selectedAlbum.mediaType === "video" ? (
                /* NEW: Video grid — 16:9 tiles, viewfinder brackets on hover,
                   tap/click opens the full-screen player */
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 sm:gap-6 xl:gap-8 mt-8 sm:mt-10">
                  {selectedAlbum.videos.map((video, idx) => (
                    <button
                      ...
                      onMouseLeave={(e) =>
                        resetPreview(e.currentTarget.querySelector("video"))
                      }
                      aria-label={`Play ${video.title || selectedAlbum.title}`}
                    ...
                    >
                      <video
                        ref={(el) => (previewVideoRefs.current[idx] = el)}
                        src={video.src}
                        poster={video.poster}
                        muted
                        loop
                      ...
                      {/* Viewfinder brackets, matching the hero highlight card */}
                      ...
                      <span className="absolute bottom-0 left-0 right-0 flex items-end justify-between gap-3 px-4 pb-3 sm:px-5 sm:pb-4 pointer-events-none">
                        <span className="text-warm-white text-base sm:text-lg font-light leading-tight">
                          {video.title || `${selectedAlbum.title} ${idx + 1}`}
                        </span>
                        {formatDuration(durations[video.src]) && (
                ...
                /* Photo Grid — Pinterest/masonry style, images keep their natural aspect ratio, no cropping */
                <div
      ...
      {/* NEW: full-screen video player — sits above the WS overlay. Click the
          ...
          >
            <video
              ...
              onLoadedMetadata={(e) => {
                const secs = e.currentTarget.duration;
                setDurations((prev) =>
                  prev[activeVideo.src]
                    ? prev
                    : { ...prev, [activeVideo.src]: secs },
                );
              }}
              className="w-full max-h-[72vh] sm:max-h-[78vh] rounded-lg bg-black object-contain"
            />

            {/* Viewfinder brackets around the player */}
      ...
      {isLoaded && !prefersReducedMotion && !heroComplete && (
